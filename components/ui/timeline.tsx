...
export const Timeline = ({ data, title, description }: TimelineProps) => {
  ...
  return (
    <div className="w-full bg-background font-sans md:px-10" ref={containerRef}>
      <div className="mx-auto max-w-7xl px-4 pb-4 pt-20 md:px-8 lg:px-10 text-justify">
        {title && (
          <h2 className="mb-4 max-w-4xl text-lg font-bold text-foreground md:text-4xl">
            {title}
          </h2>
        )}
        {description && (
      ...
      <div ref={ref} className="relative mx-auto max-w-7xl pb-20">
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-20 md:gap-10 md:pt-80">
